      ...
      <main className="pt-32 px-6 pb-32 md:px-12 lg:px-24 max-w-7xl mx-auto">
        <div className="space-y-16 md:space-y-24">
          {roles.map((role, index) => (
            <div key={index} className="snap-start md:snap-align-none scroll-m-6 group relative bg-[#0f0f13] shadow-lg p-4 sm:p-6 md:p-12 rounded-3xl border border-elevate-paper/[0.05] hover:border-elevate-orange/50 transition-colors duration-500 max-h-[95dvh] overflow-y-auto overflow-x-hidden md:max-h-none md:overflow-visible flex flex-col justify-between">
              
              <div className="flex flex-col md:flex-row md:items-start justify-between gap-3 md:gap-8 mb-4 md:mb-12">
                <div>
                  <h2 className="text-lg sm:text-xl md:text-4xl font-black tracking-tight mb-1 md:mb-2 leading-tight">{role.title}</h2>
                </div>
                <a
                  href={getGoogleFormLink(role.title)}
                  target="_blank"
                  rel="noopener noreferrer"
                  className="inline-flex items-center justify-center shrink-0 w-full md:w-auto bg-elevate-orange text-black px-4 sm:px-8 py-2.5 md:py-4 font-black uppercase tracking-widest text-[10px] md:text-sm hover:bg-white transition-colors duration-300"
                >
                  Apply Now
                </a>
              </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4 md:gap-12 flex-1">
                <div className="space-y-4 md:space-y-8">
                  <div>
                    <h4 className="text-[9px] md:text-sm font-bold tracking-[0.1em] text-white/40 uppercase mb-1.5 md:mb-3 border-b border-white/10 pb-1 md:pb-2">Mission</h4>
                    <p className="text-[11px] md:text-base leading-snug md:leading-relaxed">{role.mission}</p>
                  </div>

                  <div>
                    <h4 className="text-[9px] md:text-sm font-bold tracking-[0.1em] text-white/40 uppercase mb-1.5 md:mb-3 border-b border-white/10 pb-1 md:pb-2">Key Responsibilities</h4>
                    <ul className="space-y-1.5 md:space-y-3">
                      {role.responsibilities.map((resp, i) => (
                        <li key={i} className="flex items-start gap-2 md:gap-3 text-[11px] md:text-base leading-snug md:leading-relaxed">
                          <span className="text-elevate-orange mt-1 md:mt-1.5 shrink-0 block w-1 h-1 md:w-1.5 md:h-1.5 rounded-full bg-elevate-orange" />
                          <span>{resp}</span>
                        </li>
                      ))}
                    </ul>
                ...
                <div className="space-y-4 md:space-y-8">
                  <div>
                    <h4 className="text-[9px] md:text-sm font-bold tracking-[0.1em] text-white/40 uppercase mb-1.5 md:mb-3 border-b border-white/10 pb-1 md:pb-2">Deliverables</h4>
                    <p className="text-[11px] md:text-base leading-snug md:leading-relaxed font-['Special_Elite',_monospace] text-elevate-paper/80">{role.deliverables}</p>
                  </div>
                  {role.tools && (
                    <div>
                      <h4 className="text-[9px] md:text-sm font-bold tracking-[0.1em] text-white/40 uppercase mb-1.5 md:mb-3 border-b border-white/10 pb-1 md:pb-2">Standard Tools</h4>
                      <p className="text-[11px] md:text-base leading-snug md:leading-relaxed font-semibold">{role.tools}</p>
                    </div>
                  )}
                </div>
              </div>
